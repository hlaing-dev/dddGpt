...
const VideoFeed = ({
  ...
  const [hasSetActiveIndex, setHasSetActiveIndex] = useState(false); // Add this state
    ...
    if (videos.length > 0) {
      const decryptAndSetVideos = async () => {
        ...
        if (!hasSetActiveIndex) {
          ...
          setHasSetActiveIndex(true); // Set the flag to true after execution
        }
      };
      decryptAndSetVideos();
    }
  }, [videos, currentActiveId, hasSetActiveIndex]); // Add `hasSetActiveIndex` as a dependency
  ...
  useEffect(() => {
    ...
    if (
      currentVideoIndex >= decryptedVideos.length - 2 &&
      !isDecrypting &&
      decryptedVideos.length > 0
    ) {
      setPage((prev: any) => prev + 1); // Load more videos
    }
  }, [currentVideoIndex, decryptedVideos.length, isDecrypting]);
  ...
  //     setSwipeTransition(false);
  //     const prevIndex =
  //       (currentVideoIndex - 1 + decryptedVideos.length) %
  //       decryptedVideos.length;
  ...
  //     setSwipeTransition(false);
  //     const nextIndex = (currentVideoIndex + 1) % decryptedVideos.length;
  ...
  const slideRenderer = ({ index, key }: { index: number; key: number }) => {
    const videoIndex = mod(
      currentVideoIndex + index - 1,
      decryptedVideos.length
    );
    const item = decryptedVideos[videoIndex];
    ...
    <>
      ...
      >
        ...
        {isDecrypting && decryptedVideos?.length === 0 ? (
          ...
          <div className={`h-screen bg-[#16131C]`}>
            <div className={`flex justify-center items-center w-full`}>
              <div className="max-w-[1024px] home-main w-full">
                <div className="fixed top-3 left-0 flex gap-2 items-center w-full z-[9999]">
                  <button className="p-3" onClick={handleBack}>
...
//       // Swiped left (previous video)
//       if (currentVideoIndex === 0) {
//         // Already at first video, don't go further
//         setSwipeIndex(1);
//         return;
//       }
//       setSwipeTransition(false);
...
//       // Swiped right (next video)
//       if (currentVideoIndex === decryptedVideos.length - 1) {
//         // Already at last video, don't go further
//         setSwipeIndex(1);
//         return;
//       }
//       setSwipeTransition(false);
...
//   // Slide renderer for virtualized swipeable views
//   const slideRenderer = ({ index, key }: { index: number; key: number }) => {
//     // Calculate the actual video index based on current position
//     let videoIndex;
//     if (index === 0) {
//       videoIndex = currentVideoIndex - 1; // previous video
//     } else if (index === 1) {
//       videoIndex = currentVideoIndex; // current video
//     } else {
//       videoIndex = currentVideoIndex + 1; // next video
//     }

//     // Handle edge cases
//     if (videoIndex < 0 || videoIndex >= decryptedVideos.length) {
//       return null; // Don't render anything for out-of-bounds indices
//     }
...
//       {isDecrypting && decryptedVideos?.length === 0 ? (
//         <div
//           className="app bg-[#16131C]"
//           style={{
//             height: isHome ? "calc(100dvh-76px)" : "100dvh",
//           }}
//         >
//           <div style={{ textAlign: "center", padding: "20px" }}>
//             <div className="heart">
...
//               {currentVideo && (
//                 <div className="app__videos h-screen overflow-hidden relative">
//                   <VirtualizedSwipeableViews
//                     axis="y"
//                     index={swipeIndex}
//                     onChangeIndex={handleSwipeChange}
...
//                     slideCount={3}
//                     // slideCount={
//                     //   currentVideoIndex === decryptedVideos.length - 1 ? 2 : 3
//                     // } // Only show current and previous when at last video
//                     slideRenderer={slideRenderer}
//                     animateTransitions={swipeTransition}
//                     style={{
//                       height: "calc(100dvh - 76px)",
